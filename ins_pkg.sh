...
install_js_pkgs_initial() {
    if [ -d "${SCR_PATH}/yxnwh_checkin_main" ]; then
        cd "${SCR_PATH}/yxnwh_checkin_main" &&
            cp "${REPO_PATH}/yxnwh_checkin_main/package.json" "${SCR_PATH}/yxnwh_checkin_main/package.json"
        ...
        install 1 "npm install -g package-merge" "$(echo "$(npm install -g package-merge && npm ls -g package-merge)" | grep -cE '(empty)|ERR')" &&
            ...
            node -e \
                "const merge = require('package-merge');
                 const fs = require('fs');
                 const dst = fs.readFileSync('/ql/repo/yxnwh_checkin_main/package.json');
    ...
    if command -v cpm >/dev/null 2>&1; then
        ...
        if ! command -v cpm >/dev/null 2>&1; then
            if [ -f ./cpm ]; then
                ...
                cp -f /ql/repo/yxnwh_checkin_main/cpm ./ && chmod +x cpm && ./cpm --version
                if [ ! -f ./cpm ]; then
